...
RSpec.describe "Manage::MoneyIncoming received history", type: :request do
  ...
  it "lists collected payments under Received, linking to their payment pages" do
    ...
    body = response.body
    expect(body).to include("Received")
    expect(body).to include("$250.00 collected all time")
  ...
  it "scopes the history to the production page's own money" do
  ...
  it "shows no Received section while nothing has been collected" do
    create(:contract_payment, contract: contract, direction: "incoming", amount: 100,
                              due_date: 1.week.from_now.to_date)

    get manage_money_incoming_path
    expect(response.body).not_to include("collected all time")
  end
end

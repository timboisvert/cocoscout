class Manage::GroupsController < Manage::ManageController
  before_action :set_group, only: %i[show update_availability add_to_cast remove_from_cast remove_from_organization destroy]
  ...
  def update_availability
    ...
    if updated_count > 0
      redirect_to manage_group_path(@group), notice: "Availability updated for #{updated_count} #{'show'.pluralize(updated_count)}"
    else
  ...
  def remove_from_organization
    ...
    redirect_to manage_directory_path, notice: "#{@group.name} was removed from #{Current.organization.name}", status: :see_other
  end

...
RSpec.describe 'Public person profile shoutouts parity', type: :system do
  ...
  let!(:person) do
    create(:person, user: person_owner, email: person_owner.email_address, name: 'Taylor Swift',
                    public_key: 'taylor-swift')
  end
  ...
  it 'matches shoutouts UI/CTA like group profile (signed-out shows sign-in CTA)' do
    ...
    expect(page).to have_link("Give #{person.name} a Shoutout",
                              href: my_shoutouts_path(tab: 'given', shoutee_type: 'Person', shoutee_id: person.id))
  end
end

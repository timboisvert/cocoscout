...
export default class extends Controller {
    ...
    connect() {
        ...
        window.addEventListener("show-actions:open-move-modal", this.boundOpenFromEvent)
    }

    disconnect() {
        window.removeEventListener("show-actions:open-move-modal", this.boundOpenFromEvent)
    }
    ...
    renderProductions() {
      ...
      <label class="flex items-center gap-3 p-3 rounded-lg border border-gray-200 hover:border-pink-300 hover:bg-pink-50 cursor-pointer transition-colors production-option" data-production-id="${prod.id}">
        <input type="radio" name="production_selection" value="${prod.id}" class="h-4 w-4 text-pink-600 border-gray-300 focus:ring-pink-500 accent-pink-600" data-action="change->show-actions#selectProduction">

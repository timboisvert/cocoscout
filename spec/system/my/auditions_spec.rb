...
RSpec.describe 'My::Auditions', type: :system do
  ...
  let(:audition_cycle) do
    create(:audition_cycle, production: production, form_reviewed: true, finalize_audition_invitations: true)
  end
  ...
  describe 'when user has upcoming auditions' do
    ...
    let!(:upcoming_audition) do
      create(:audition, person: person, audition_request: audition_request, audition_session: upcoming_session)
    end
  ...
  describe 'when user has past auditions' do
    ...
    let!(:past_audition) do
      create(:audition, person: person, audition_request: audition_request, audition_session: past_session)
    end
  ...
  describe 'multiple auditions' do
    ...
    let!(:audition1) do
      create(:audition, person: person, audition_request: audition_request, audition_session: session1)
    end
    let!(:audition2) do
      create(:audition, person: person, audition_request: audition_request, audition_session: session2)
    end

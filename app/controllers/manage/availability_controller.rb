class Manage::AvailabilityController < Manage::ManageController
  ...
  def request_availability
    ...
    @members_needing_update = @availability_data.select do |member|
  ...
  def handle_request_availability
    ...
    groups.each do |group|
      ...
      personalized_message = generate_personalized_message(pending_shows, message_template)
      Manage::AvailabilityMailer.request_availability_for_group(group, @production, personalized_message).deliver_later
      email_count += 1
    end

    redirect_to manage_production_availability_index_path(@production), notice: "Availability request sent to #{email_count} #{'recipient'.pluralize(email_count)}"
  end

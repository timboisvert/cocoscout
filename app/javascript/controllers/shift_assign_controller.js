...
export default class extends Controller {
    ...
    static values = {
        ...
        shiftTimes: Object,          // { "<shiftId>": { starts_at, ends_at, role, day, time_range } }
    ...
    pick(event) {
        ...
        if (conflicts.length > 0) {
            this.showCollision(id, name, conflicts)
            return
        }
    ...
    showCollision(personId, personName, conflicts) {
        ...
        if (this.hasCollisionBodyTarget) {
            ...
            this.collisionBodyTarget.innerHTML = `
    ...
    renderPeople(people, assignedIds) {
        ...
        const cards = visible.map(p => {
            ...
            const badge = isAssigned
                ? `<span class="absolute top-1.5 right-1.5 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-green-100 text-green-700 text-[10px] font-medium">Assigned</span>`
                : (unavailable ? `<span class="absolute top-1.5 right-1.5 inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-red-100 text-red-700 text-[10px] font-medium">Unavailable</span>` : "")

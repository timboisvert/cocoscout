      ...
      msgs.each do |m|
        label =
          if m.sent_as_production_team?
            ...
            # Automated notifications (system_generated OR message_type :system)
            # hide whichever account technically sent them (e.g. an anonymous mic
            # suggestion falls back to a superadmin as sender). MUST mirror
            # Message#sender_name — this is the compact-inbox copy of that logic,
            # and forgetting it here is why system messages showed "from <some
            # admin>" in the list while the thread said otherwise.

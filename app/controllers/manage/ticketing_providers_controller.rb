...
module Manage
  class TicketingProvidersController < Manage::ManageController
    ...
    def new
      @ticketing_provider = Current.organization.ticketing_providers.build
      @provider_types = Ticketing::ServiceFactory.available_providers
    end

    def create
      @ticketing_provider = Current.organization.ticketing_providers.build(ticketing_provider_params)

      if @ticketing_provider.save
        # Test the connection if we have credentials
        if @ticketing_provider.has_credentials?
          ...
          if result[:success]
            ...
            redirect_to manage_ticketing_provider_path(@ticketing_provider),
                        notice: "#{@ticketing_provider.provider_display_name} connected successfully!"
          else
            redirect_to manage_ticketing_provider_path(@ticketing_provider),
                        alert: "Provider created but connection failed: #{result[:error]}"
          end
        else
          redirect_to manage_ticketing_provider_path(@ticketing_provider),
                      notice: "Provider created. Add credentials to complete setup."
        end
      else
        @provider_types = Ticketing::ServiceFactory.available_providers
